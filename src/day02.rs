...
pub fn solve(path: &str, part2: bool) -> i64 {
    ...
    for segment in read_to_string(path).unwrap().split(',') {
        ...
        if !part2 {
            ...
            let mut digit = 1;
            loop {
                let mut total: i64 = format!("{}{}", digit, digit).parse().unwrap();
                if total > last_id { break; }
                while total <= last_id {
                    if total >= first_id { results.insert(total); }
                    total = format!("{}{}", total, digit).parse().unwrap();
                }
                digit += 1;
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_example_p2() {
        let result = solve("examples/day02.txt", true);
        assert_eq!(result, 4174379265);
    }

    #[test]
    fn test_input_p2() {
        let result = solve("inputs/day02.txt", true);
        assert_eq!(result, 15704845910);
    }
}
